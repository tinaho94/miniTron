...
public:
	...
	bool begin(unsigned long baudRate = 115200, esp8266_serial_port serialPort = ESP8266_HARDWARE_SERIAL);
	...
	int16_t disconnect();
	IPAddress localIP();
	int16_t setSoftAP(const char* ssid, const char* pwd, int channelId, esp8266_encryption enc);
	int16_t getSoftAP(char* ap);
	int16_t getSoftAPMAC(char* ap);
	/////////////////////
	// TCP/IP Commands //
	/////////////////////
	int16_t status();
