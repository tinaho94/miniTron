...
int16_t ESP8266Class::setSoftAP(const char* ssid, const char* pwd, int channelId, esp8266_encryption ecn) {
	...
	// Serial.print("AT");
	// Serial.print(ESP8266_AP_CONFIG);
	// Serial.print("=\"");
	// Serial.print(ssid);
	// Serial.print("\"");
	// Serial.print(',');
...
int16_t ESP8266Class::getSoftAP(char * ssid)
{
	sendCommand(ESP8266_AP_CONFIG, ESP8266_CMD_QUERY); // Send "AT+CWSAP?"
	...
	if (rsp > 0)
	{
		...
		// Look for "+CWSAP"
		char * p = strstr(esp8266RxBuffer, ESP8266_AP_CONFIG);
		if (p != NULL)
		{
			//Serial.print(p);
			p += strlen(ESP8266_AP_CONFIG) + 2;
			//Serial.print(p);
...
IPAddress ESP8266Class::localIP()
{
	...
	if (rsp > 0)
	{
		...
		char * p = strstr(esp8266RxBuffer, "APIP");
		if (p != NULL)
		{
			IPAddress returnIP;
			...
			// for STAIP 
			// p += 7
			p += 6; // Move p seven places. (skip APIP,")
...
int16_t ESP8266Class::localMAC(char * mac)
{
	...
	int16_t rsp = readForResponse(RESPONSE_OK, 2000); //COMMAND_RESPONSE_TIMEOUT);
...
int16_t ESP8266Class::getSoftAPMAC(char* mac) {

	sendCommand(ESP8266_AP_MAC, ESP8266_CMD_QUERY);
	int16_t rsp = readForResponse(RESPONSE_OK, 2000);

	if(rsp > 0) {
		// Look for +CIPAPMAC
		char* p = strstr(esp8266RxBuffer, ESP8266_AP_MAC);
		if(p != NULL) {
			p += strlen(ESP8266_AP_MAC) + 2;
			char* q = strchr(p, '"');
			if(q == NULL) return ESP8266_RSP_UNKNOWN;
			strncpy(mac, p, q - p);
			return 1;
...
int16_t ESP8266Class::close(uint8_t linkID)
{
	...
	return readForResponses(RESPONSE_OK, RESPONSE_ERROR, 5000);
}
...
int16_t ESP8266Class::configureTCPServer(uint16_t port, uint8_t create)
{
	...
	// _serial->print("AT");
	// _serial->print(ESP8266_SERVER_CONFIG);
	// _serial->print("=\"");
	// _serial->print(1);
	// _serial->print("\"");
	// _serial->print(',');
...
int16_t ESP8266Class::readForResponse(const char * rsp, unsigned int timeout)
{
	...
	{
		int n;
		if (n = _serial->available()) // If data is available on UART RX
		{
			int i;
			for(i=0;i<n;i++)
			{
...
char * ESP8266Class::searchBuffer(const char * test)
{
	...
	int testLen = strlen((const char *)esp8266RxBuffer);

	// If our buffer isn't full, just do an strstr
	if (bufferLen < ESP8266_RX_BUFFER_LEN && bufferLen >= testLen)
